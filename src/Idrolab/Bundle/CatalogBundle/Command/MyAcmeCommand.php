...
//        $output->writeln("<info>End of my acme command<info>");
    ...
    $attribute_title = $this->findAttribute('title');
    if (!$attribute_title) {
      // create an attribute
      $attribute_title = $am->createAttribute('pim_catalog_text');
      $attribute_title->setCode('title');
      $this->saveAttribute($attribute_title);
    }
    $opt1 =$this->findOption('color.purple');
    if(!$opt1){
      $opt1 = $am->createAttributeOption();
      $opt1->setCode('purple');
//      $attribute_option->addOption($opt1);
    }
    var_dump($opt1->getCode());
    
    $attribute_option = $this->findAttribute('color');
    if (!$attribute_option){
      $attribute_option = $am->createAttribute('pim_catalog_simpleselect');
      $attribute_option->setCode('color');

      $opt1 =$this->findOption('purple');
      if(!$opt1){
        $opt1 = $am->createAttributeOption();
        $opt1->setCode('purple');
        $attribute_option->addOption($opt1);
    ...
    $this->saveAttribute($attribute_sku);
    ...
    $this->addValueToProduct($pm, $attribute_title, $product);
...
//    $product->getValue('color')->setOption($opt2);
  ...
  private function findOption($option_code) {

    $repository_attribute = $this->getContainer()->get('pim_catalog.repository.attribute_option');
    return $repository_attribute->findByReference($option_code);
  }
